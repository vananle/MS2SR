...
def g(segments, i, j, k, u, v):
    ...
    value = 0
    if len(segments[i, j][k][0]) != 0 and ((u, v) in segments[i, j][k][0] or (v, u) in segments[i, j][k][0]):
        value += 1

    if len(segments[i, j][k][1]) != 0 and ((u, v) in segments[i, j][k][1] or (v, u) in segments[i, j][k][1]):
        value += 1
